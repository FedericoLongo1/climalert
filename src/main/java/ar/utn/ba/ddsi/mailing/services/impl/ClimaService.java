...
import ar.utn.ba.ddsi.mailing.models.entities.Pais;
import ar.utn.ba.ddsi.mailing.models.entities.Region;
    ...
    public Mono<Void> actualizarClimaCiudades() {
        return Flux.fromArray(CIUDADES_ARGENTINA)
            ...
            .flatMap(clima -> {
                ...
                logger.info("Clima actualizado para: {}", clima.getUbicacion().getCiudad().getNombre());
                return Mono.empty();
            })
            .onErrorResume(e -> {
        ...
        return webClient.get()
            ...
            .map(response -> {
                ...
                Ubicacion ubicacion = new Ubicacion(
                        new Ciudad(response.getLocation().getName()),
                        new Region(response.getLocation().getRegion()),
                        new Pais(response.getLocation().getCountry())
                );
                clima.setUbicacion(ubicacion);
                Temperatura temperaturaCelcius = new Temperatura(response.getCurrent().getTemp_c());
                clima.setTemperatura(temperaturaCelcius);
